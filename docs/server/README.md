# API Server — `apps/server/apiServer`

Fastify 기반 REST API 서버입니다.  
PostgreSQL(Prisma) + Redis(ioredis)를 연동하며 JWT 인증과 RBAC 권한 제어를 제공합니다.

> **서버 디렉토리 구조**  
> `apps/server/` 하위에는 여러 서버 앱이 나란히 위치합니다.  
> 현재는 `apiServer`만 존재하며, 향후 `notificationServer`, `MCPServer` 등이 추가될 예정입니다.
...
| `UPLOAD_STORAGE` | 파일 저장 방식 | `local` 또는 `s3` |
| `UPLOAD_MAX_FILE_SIZE_MB` | 업로드 최대 크기 (MB) | `50` |
...
# 빌드 결과: apps/server/apiServer/dist/
```
...
`apps/server/apiServer/Dockerfile`은 멀티스테이지 빌드로 최적화되어 있습니다.
