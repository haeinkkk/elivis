...
import { addProjectMemberAction } from "@/app/actions/projects";
import { searchUsersForTeamAction, type SearchableUser } from "@/app/actions/teams";
...
export function ProjectAddMemberModal({
    ...
    const handlePick = async (user: SearchableUser) => {
        setError("");
        setAddingId(user.id);
        const r = await addProjectMemberAction(projectId, user.id, "MEMBER");
        setAddingId(null);
        if (r.ok) {
            onSuccess(r.project);
            onClose();
