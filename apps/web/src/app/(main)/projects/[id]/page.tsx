...
type MockTask = { id: string; title: string; status: TaskStatus; assignee?: string; dueDate?: string };
const MOCK_RECENT_TASKS: MockTask[] = [
  { id: "1", title: "기획서 초안 작성", status: "완료", assignee: "김철수", dueDate: "2025-03-15" },
  { id: "2", title: "API 스펙 정의", status: "진행중", assignee: "이영희", dueDate: "2025-03-22" },
  { id: "3", title: "디자인 시안 검토", status: "검토", assignee: "박민수", dueDate: "2025-03-20" },
  { id: "4", title: "DB 스키마 설계", status: "진행중", assignee: "정수진", dueDate: "2025-03-25" },
...
const MOCK_IN_PROGRESS_TASKS: MockTask[] = [
  { id: "2", title: "API 스펙 정의", status: "진행중", assignee: "이영희", dueDate: "2025-03-22" },
  { id: "4", title: "DB 스키마 설계", status: "진행중", assignee: "정수진", dueDate: "2025-03-25" },
  { id: "6", title: "프론트엔드 컴포넌트 개발", status: "진행중", assignee: "김철수", dueDate: "2025-03-28" },
  { id: "7", title: "백엔드 API 구현", status: "진행중", assignee: "박민수", dueDate: "2025-03-30" },
  { id: "8", title: "테스트 케이스 작성", status: "진행중", assignee: "이영희", dueDate: "2025-04-02" },
];
...
function getStatusCounts(tasks: MockTask[]): { status: TaskStatus; count: number; color: string }[] {
...
const AVATAR_PLACEHOLDER_COLORS = ["#78716c", "#3b82f6", "#059669", "#d97706", "#7c3aed", "#dc2626"];
...
function AvatarStack({ participants, size = "md" }: { participants: ProjectUser[]; size?: "sm" | "md" }) {
  ...
  return (
    <div className="flex items-center -space-x-2" aria-label={`참여 인원 ${participants.length}명`}>
  ...
  return (
    <div className="flex min-h-full w-full flex-col">
          ...
          {project && (
            ...
            >
              <div className="flex cursor-default items-center gap-3 rounded-lg px-2 py-1.5 transition-colors hover:bg-stone-100">
                ...
                {project.participants.length > 0 && <AvatarStack participants={project.participants} size="md" />}
              </div>
                    ...
                    {project.participants.map((user) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1 truncate">
                          ...
                          {user.userId && (
                            <p className="truncate text-xs text-stone-500">{user.userId}</p>
                          )}
                        </div>
                      </li>
                    ))}
      ...
      <div className="border-b border-stone-200 bg-white/95">
        <nav className="flex gap-0 overflow-x-auto px-4 sm:px-5 md:px-6" aria-label="프로젝트 서브메뉴">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              type="button"
      ...
      <div className="min-h-0 flex-1 p-4 sm:p-5 md:p-6">
        ...
        {activeTab === "list" && (
          <div className="rounded-xl border border-stone-200 bg-white p-6">
            <h2 className="text-base font-semibold text-stone-800 sm:text-lg">📋 업무</h2>
            <p className="mt-1 text-sm text-stone-500">엑셀처럼 한 줄씩 업무를 관리합니다.</p>
            <div className="mt-6 overflow-x-auto">
              <table className="w-full min-w-[400px] text-left text-sm">
                <thead>
                  <tr className="border-b border-stone-200">
                    <th className="pb-2 pr-4 font-medium text-stone-600">할 일</th>
                    <th className="pb-2 pr-4 font-medium text-stone-600">상태</th>
                    <th className="pb-2 font-medium text-stone-600">담당</th>
                  </tr>
                </thead>
                <tbody className="text-stone-600">
                  <tr className="border-b border-stone-100"><td className="py-2 pr-4">데모 행 1</td><td className="py-2 pr-4">대기</td><td className="py-2">—</td></tr>
                  <tr className="border-b border-stone-100"><td className="py-2 pr-4">데모 행 2</td><td className="py-2 pr-4">진행중</td><td className="py-2">—</td></tr>
                  <tr className="border-b border-stone-100"><td className="py-2 pr-4">데모 행 3</td><td className="py-2 pr-4">완료</td><td className="py-2">—</td></tr>
                </tbody>
              </table>
            </div>
            <p className="mt-4 text-xs text-stone-400">(데모) 추후 실제 업무 데이터 연동</p>
          </div>
        )}

        {activeTab === "board" && (
          <div className="rounded-xl border border-stone-200 bg-white p-6">
            <h2 className="text-base font-semibold text-stone-800 sm:text-lg">🧱 보드</h2>
            <p className="mt-1 text-sm text-stone-500">칸반 스타일로 업무를 관리합니다.</p>
            <div className="mt-6 flex gap-4 overflow-x-auto pb-2">
              {["할 일", "진행중", "검토", "완료"].map((col) => (
                <div key={col} className="min-w-[200px] rounded-lg border border-stone-200 bg-stone-50/50 p-3">
                  <p className="text-xs font-medium uppercase tracking-wider text-stone-500">{col}</p>
        ...
        {activeTab === "calendar" && (
          <div className="rounded-xl border border-stone-200 bg-white p-6">
            <h2 className="text-base font-semibold text-stone-800 sm:text-lg">📅 캘린더</h2>
            ...
            <div className="mt-6 rounded-lg border border-stone-200 bg-stone-50/30 p-8 text-center">
              ...
              <p className="mt-2 text-sm text-stone-400">일정/마일스톤이 여기에 표시됩니다.</p>
            </div>
        ...
        {activeTab === "wiki" && (
          <div className="rounded-xl border border-stone-200 bg-white p-6">
            <h2 className="text-base font-semibold text-stone-800 sm:text-lg">📓 위키</h2>
            <p className="mt-1 text-sm text-stone-500">가이드라인, 회의록을 마크다운으로 작성합니다.</p>
            <div className="mt-6 rounded-lg border border-stone-200 bg-stone-50/30 p-6 font-mono text-sm text-stone-600">
              <pre># 가이드라인 (데모)\n\n- 회의록\n- 마크다운 문서\n- 프로젝트 노트</pre>
            </div>
            <p className="mt-4 text-xs text-stone-400">(데모) 추후 마크다운 에디터 연동</p>
          </div>
        )}

        {activeTab === "settings" && (
          <div className="rounded-xl border border-stone-200 bg-white p-6">
            <h2 className="text-base font-semibold text-stone-800 sm:text-lg">⚙️ 설정</h2>
            <p className="mt-1 text-sm text-stone-500">멤버, 커스텀 필드, 자동화 규칙을 관리합니다.</p>
            <ul className="mt-6 space-y-3 text-sm text-stone-600">
              <li className="flex items-center gap-2 rounded-lg border border-stone-100 bg-stone-50/50 px-4 py-3">멤버 초대 (데모)</li>
              <li className="flex items-center gap-2 rounded-lg border border-stone-100 bg-stone-50/50 px-4 py-3">커스텀 필드 (데모)</li>
              <li className="flex items-center gap-2 rounded-lg border border-stone-100 bg-stone-50/50 px-4 py-3">자동화 규칙 (데모)</li>
            </ul>
...
// ——— SVG 도넛 차트 (완료율) ———
function DonutChart({ percent, size = 120, strokeWidth = 12, color = "#22c55e" }: { percent: number; size?: number; strokeWidth?: number; color?: string }) {
          ...
          <div className="flex flex-1 flex-col items-center justify-center sm:px-6">
            <p className="flex items-baseline gap-2">
              {remainingDays === null ? (
                daysSinceStart !== null ? (
                  <>
                    <span className="text-4xl font-bold text-stone-900 sm:text-5xl">{daysSinceStart}일</span>
                    <span className="text-xl font-bold text-stone-900 sm:text-2xl">지났어요!</span>
                  </>
                ) : (
                  <span className="text-xl font-medium text-stone-500">종료일 없음</span>
                )
              ) : remainingDays > 0 ? (
                <>
                  <span className="text-4xl font-bold text-stone-900 sm:text-5xl">{remainingDays}일</span>
                  <span className="text-xl font-bold text-stone-900 sm:text-2xl">남았어요!!</span>
                </>
              ) : remainingDays === 0 ? (
                <>
                  <span className="text-4xl font-bold text-amber-600 sm:text-5xl">0일</span>
                  <span className="text-xl font-bold text-stone-900 sm:text-2xl">오늘 마감이에요!</span>
                </>
              ) : (
                <>
                  <span className="text-3xl font-bold text-amber-600 sm:text-4xl">마감일 지남</span>
                </>
              )}
            </p>
          </div>
          ...
          {progressPercent !== null && (
            <div className="flex flex-col items-center">
              <div className="relative">
                <DonutChart percent={progressPercent} size={120} strokeWidth={12} color="#78716c" />
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        ...
        >
          ...
          <div className="mt-6 flex flex-wrap items-center gap-8">
            <div className="flex flex-col items-center">
              <div className="relative">
                <DonutChart percent={completionPercent} size={140} strokeWidth={14} color="#22c55e" />
            ...
            <div className="flex-1 space-y-2 min-w-[140px]">
              <p className="text-sm text-stone-600">
                <span className="font-semibold text-green-600">{completedCount}개</span> 완료 / 전체 {totalTasks}개
              </p>
          ...
          <div className="mt-6 space-y-4">
            {statusCounts.map(({ status, count, color }) => (
              <div key={status} className="flex items-center gap-3">
                <span className="w-16 shrink-0 text-sm font-medium text-stone-600">{status}</span>
                <div className="min-w-0 flex-1">
        ...
        >
          ...
          <ul className="mt-4 space-y-2">
            {inProgressTasks.slice(0, 5).map((task) => (
              ...
              >
                <span className="min-w-0 flex-1 truncate text-sm font-medium text-stone-800">{task.title}</span>
                <span className="shrink-0 text-xs text-stone-500">{task.assignee}</span>
              </li>
            ))}
          </ul>
          {onSeeMoreTasks && (
        ...
        <ul className="mt-4 space-y-2">
          {recentTasks.map((task, i) => (
            <li
              ...
              style={{ animation: `overview-fade-in 0.35s ease-out ${0.35 + i * 0.05}s both` }}
            >
              <span className="min-w-0 flex-1 truncate font-medium text-stone-800">{task.title}</span>
              <span
              ...
              <span className="hidden shrink-0 text-sm text-stone-500 sm:inline">{task.assignee}</span>
            </li>
          ))}
        </ul>
        {onSeeMoreTasks && (
